...
class ServerRouteTests(unittest.TestCase):
    ...
    def test_dashboard_uses_database_invoice_queue_not_generated_samples(self) -> None:
        ...
        self.assertIn('data-document-mode="pdf" aria-pressed="true"', response)
        self.assertIn('data-document-mode="json" aria-pressed="false"', response)
        self.assertIn('data-document-view="json" hidden', response)
        self.assertIn("Decision data", response)
        self.assertIn('&quot;decision&quot;: &quot;needs_review&quot;', response)
        self.assertIn('&quot;id&quot;: &quot;amount_match&quot;', response)
        self.assertIn('&quot;normalized_vendor&quot;: &quot;DATABASE VENDOR LLC&quot;', response)
        self.assertIn("setDocumentMode", response)
        self.assertNotIn('/api/mail/pdfs/20', response)
        self.assertNotIn(">AP context<", response)
        self.assertNotIn(">AP review<", response)
        self.assertIn('&quot;type&quot;: &quot;ap_context_records&quot;', response)
