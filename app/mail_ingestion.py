...
def _message_text(subject: str | None, sender: str | None, filename: str | None) -> str:
    return " ".join(value or "" for value in [subject, sender, filename]).lower()
...
def candidate_reason(*, subject: str | None, sender: str | None, filename: str | None) -> str:
    text = _message_text(subject, sender, filename)
    if any(term in text for term in ["invoice", "bill", "receipt", "tax", "gst"]):
        return "invoice_hint"
...
class MailIngestionService:
    ...
    def process_gmail_message(self, *, account_id: int, message_id: str) -> None:
        ...
        message_row = self.repo.upsert_message(
            ...
            has_attachments=bool(gmail_pdf_parts(payload)),
            metadata={
        ...
        for part in gmail_pdf_parts(payload):
            body = part.get("body") if isinstance(part.get("body"), dict) else {}
            ...
            attachment_row = self.repo.upsert_attachment(
                ...
                candidate_reason=candidate_reason(subject=subject, sender=sender, filename=filename),
                metadata={
